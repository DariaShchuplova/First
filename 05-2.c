...
#include <stdlib.h>
int main(){
	//Создаем массив из 2ух элементов для хранения файловых дескрипторов для чтения и для записи
	int fd1[2], fd2[2];
	pid_t result;
	size_t size;
	...
	result = fork();
	if (result<0) {
		printf("Ошибка выполнения выполнения fork()\n");
		exit(-1);
	}
	else if (result>0) {
		if (close(fd1[0]<0)) {
		...
		//Проверака количства байт, записанных в строку с помощью write()
		if(size != 13){
			printf("У родителя не получается записать строку\n");
			exit(-1);
		}
		...
		//Проверка количества байт, записаных в строку с помощью write()
		if (size != 14) {
			printf("Не получилось  записать строку\n");
			exit(-1);
		}
		printf("Процесс-ребенок записал информациб в pipe\n");
