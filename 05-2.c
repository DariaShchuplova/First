...
//Создаем массив из 2ух элементов для хранения файловых дескрипторов для чтения и для записи
	...
	//Пробуем записать в pipe 14 байт из нашего массива, т.е. всю строку "Hello, world!" вместе с признаком конца строки
	...
	//Проверка количства байт, прочитанных из pipe() с помощью read()
	if(size < 0){
